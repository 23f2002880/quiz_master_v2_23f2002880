...
def login_admin():
    ...
    if admin and admin.password == data['password']:  # Ideally use hashing
...
def edit_chapter(chapter_id):
    ...
    if "subject_id" in data:
        new_subject = Subject.query.get(int(data["subject_id"]))
        if new_subject:
...
@admin_routes.route("/api/chapters/by_subject", methods=["GET"])
@jwt_required()
